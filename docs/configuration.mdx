...
| relative_paths | `false`      | Controls whether Argon should work on relative paths instead of absolute ones. Only useful when working with tools that require this such as [darklua](https://darklua.com/). This setting affects internal processing, sourcemap generation and user output thus is recommended to be disabled.                                                                                                  |

...
Many property types have an **implicit** and **explicit** format. While most of the time it's easier to specify properties implicitly, explicit values are useful when you need to specify a property type that can't be represented in a simple way such as [ColorSequence](#colorsequence) or [NumberSequence](#numbersequence). It also helps with specifying instance [Attributes](#attributes).
...
```json
{
  "$properties": {
    ...
    "ExplicitExample": {
      "Color3": [0.6392156862745098, 0.6352941176470588, 0.6470588235294118]
    }
  }
}
```
